...
#ifndef SIGNALSTAT_HPP
#define SIGNALSTAT_HPP
...
public:
  ...
  /// The basic algorithm
  static void visibilityGraphBasic(std::vector<double> &t, std::vector<double> &y,
    ...
    for (int ia = 0; ia < itmax; ia++) {
      ...
      for (int ib = ia + 2; ib <= itmax; ib++) {
        ...
        if (connect == true) { graph.push_back(std::pair<int, int>(ia, ib)); }
      }
    }
  }
  ...
  static void visibilityGraphFast(std::vector<double> &t, std::vector<double> &y,
                                  std::vector<std::pair<int, int>> &graph) {
    graph.clear();
    int itmax = t.size() - 1;
    for (int ia = 0; ia < itmax; ia++) {
  ...
  static void WeierstrassFunction(double g, double h, double x0, double xf, int nt, std::vector<double> &t,
                                  std::vector<double> &y) {
...
#endif /* end of include guard: SIGNALSTAT_HPP */
