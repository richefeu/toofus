...
// This is the class that implement the least square computation
// to be given to be minimized (with Powell)
struct LeastSquare {
  paramFunc func;
...
struct LSfit {
  LeastSquare LS;
  static std::map<const char *, std::function<paramFunc(std::vector<double> &)>> FuncDataBase;

  LSfit() {}
  LSfit(std::vector<double> &X, std::vector<double> &Y, std::vector<double> &Params, paramFunc Func) {
    run(X, Y, Params, Func);
  }

  LSfit(std::vector<double> &X, std::vector<double> &Y, std::vector<double> &Params, const char *FuncName) {
...
std::map<const char *, std::function<paramFunc(std::vector<double> &Params)>> LSfit::FuncDataBase = {
    {"linear",
     [](std::vector<double> &Params) -> paramFunc {
       ...
       return [](std::vector<double> &p, double x) -> double { return p[0] + p[1] * x; };
     }},
    ...
    {"poly2",
     [](std::vector<double> &Params) -> paramFunc {
       ...
       return [](std::vector<double> &p, double x) -> double { return p[0] + p[1] * x + p[2] * x * x; };
     }},
    ...
    {"power",
     [](std::vector<double> &Params) -> paramFunc {
       ...
       return [](std::vector<double> &p, double x) -> double { return p[0] * pow(x, p[1]); };
     }},

    {"exponential", [](std::vector<double> &Params) -> paramFunc {
       if (Params.size() < 2) {
         Params.resize(2);
         Params[0] = 1.0;
         Params[1] = 1.0;
       }
       return [](std::vector<double> &p, double x) -> double { return p[0] * exp(p[1] * x); };
     }}};

// ===========================================================================================
void LSfit::add_trigo() {
  LSfit::FuncDataBase.insert({"cos", [](std::vector<double> &Params) -> paramFunc {
                                if (Params.size() < 4) {
                                  Params.resize(4);
                                  Params[0] = 0.0;
                                  Params[1] = 1.0;
...
void LSfit::add_distributions() {
  LSfit::FuncDataBase.insert({"cauchy_cdf", [](std::vector<double> &Params) -> paramFunc {
                                if (Params.size() < 2) {
                                  Params.resize(2);
  ...
  LSfit::FuncDataBase.insert({"cauchy_pdf", [](std::vector<double> &Params) -> paramFunc {
                                if (Params.size() < 2) {
                                  Params.resize(2);
                                  Params[0] = 0.0; // x0, location
  ...
  LSfit::FuncDataBase.insert({"gaussian_pdf", [](std::vector<double> &Params) -> paramFunc {
                                if (Params.size() < 2) {
                                  Params.resize(2);
  ...
  LSfit::FuncDataBase.insert({"gaussian_cdf", [](std::vector<double> &Params) -> paramFunc {
                                if (Params.size() < 2) {
                                  Params.resize(2);
                                  Params[0] = 0.0; // mu, mean
                                  Params[1] = 1.0; // sigma, stddev
...
int main() {
  ...
  // std::vector<double> y = {0, 1., 1.99, 3, 4.01, 5.1}; // y = x
   std::vector<double> y = {1, 3, 5, 7, 9, 11}; // y = 2x + 1
  //std::vector<double> y = {1, 2, 1, -2, -7, -14}; // y = -x^2 + 2x + 1
