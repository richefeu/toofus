...
template <typename T> void loadDataTable(const char *filename, std::vector<std::vector<T>> &DataTable) {
...
template <typename T> void saveDataTable(const char *filename, std::vector<std::vector<T>> &DataTable) {
  ...
  for (int l = 0; l < nlin; l++) {
    for (int c = 0; c < ncol; c++) { ofs << DataTable[c][l] << ' '; }
    ofs << '\n';
  }
}
